...
- Variables conservadas: 11 (O3, CO, NO2, NOX, PM10, PM2.5, TOUT, RAINF, PRS, WSR, WDR). Descartadas: NO, RH, SO2, SR.
...
- (a) Centinelas -9999 → NaN: 1 celdas (0.0000 %). Es el código de dato ausente del datalogger, no una medición.
- (b) Topes de saturación (999/1000/1001 µg/m³) en PM → NaN: 6 celdas (0.0001 %).
- (c) Fuera del rango físico del sensor → NaN: 57 celdas (0.0009 %). Desglose: O3=1, CO=1, NOX=12, PM2.5=1, TOUT=3, WSR=39.
- (d) Fuera del rango de OPERACIÓN anual, CONSERVADOS y marcados: O3=6 (0.00 %); CO=24 (0.00 %); NOX=4 (0.00 %); TOUT=63 (0.01 %); RAINF=8 (0.00 %); PRS=7987 (1.39 %); WSR=1655 (0.29 %). No se borran porque el rango de operación es inconsistente entre años (el mínimo de PRS pasa de 687.5 a 700 mmHg y el de TOUT de -6.5 a 0 °C sin justificación física), de modo que aplicarlo al pie de la letra eliminaría mediciones válidas.
...
- Celdas imputadas: 98,382 de 6,317,773 (1.56 %).
- Celdas que permanecen NaN: 362,650 (5.74 %). Corresponden a huecos largos (paros prolongados de estación) y NO se imputan.
...
- Se excluyen del criterio IQR: WDR, RAINF. WDR es circular y su cuartil carece de sentido (produce un límite inferior de -98°); RAINF tiene 98 % de ceros, así que su IQR vale 0 y marcaría como atípica toda lluvia registrada. Para el viento se usan en su lugar las componentes viento_u y viento_v.
...
- Dataset horario limpio: 574,343 filas × 31 columnas.
