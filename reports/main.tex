...
\title{Discriminación de excedencias de ozono día-estación bajo el umbral de
51 ppb de la NOM-020-SSA1-2021\tnoteref{repo}\\
\large Comparación de análisis discriminante lineal y regresión logística en
15 estaciones del SIMA, Área Metropolitana de Monterrey}

% Nota de portada: el enlace al repositorio se pone aqui, con el gancho
% \tnotetext de elsarticle, para que salga al pie de la primera pagina.
\tnotetext[repo]{Código, notebooks y fuente de este reporte:
...
regularidad el umbral de 51 ppb del máximo diario de la media móvil de 8 horas
(MDA8). Este trabajo discrimina, a resolución día-estación, las excedencias de
ese umbral a partir de las condiciones meteorológicas y de los contaminantes
primarios del mismo día. Los registros horarios de 15 estaciones del SIMA entre
el 1 de enero de 2021 y el 30 de junio de 2025 se agregan a una fila por
día-estación; 17,002 de ellas cumplen la regla de completitud y entran al
modelado con 15 predictores. El análisis de componentes principales, ajustado
solo sobre 2021--2024 para no filtrar información de la partición de
validación, retiene tres ejes que acumulan 55.9\,\% de la varianza y admiten
lectura física: carga primaria de combustión, forzamiento fotoquímico y
estancamiento atmosférico. Sobre esas tres coordenadas y 12 indicadoras de
zona, tipo de día y temporada se ajustan una regresión logística y un análisis
discriminante lineal, evaluados sobre 2025. Ambos alcanzan un AUC de 0.893 sin
diferencia detectable entre sí, con sensibilidad de 0.831 y 0.845 y
especificidad de 0.824 y 0.811 al umbral de Youden. La primavera multiplica los
momios de excedencia por 5.11 respecto al invierno, por encima de cualquier
predictor meteorológico, y el forzamiento fotoquímico pesa más que la carga
primaria de combustión. La ausencia de mediciones de compuestos orgánicos
volátiles impide atribuir un régimen fotoquímico, de modo que los ejes se
interpretan como condiciones asociadas y no como mecanismo.
\end{abstract}

\begin{keyword}
ozono troposférico \sep análisis de componentes principales \sep regresión
logística \sep análisis discriminante lineal \sep calidad del aire \sep
...
\input{secciones/objetivo_tecnicas.tex}
...
% --- Etapa 3 ---
\input{secciones/pca.tex}
\input{secciones/analisis_factorial}
\input{secciones/regresion_logistica.tex}
\input{secciones/analisis_factorial.tex}
\input{secciones/series_tiempo_factores.tex}
\input{secciones/tecnicas_no_funcionaron.tex}
\input{secciones/declaratoria_ia.tex}

\printbibliography

% --- Anexos ---
% Los diccionarios de datos son referencia exhaustiva citada desde el cuerpo
% (ver \cref{sec:diccionario} en objetivo_tecnicas.tex y \cref{sec:diccionario-transformado}
% en transformaciones.tex); no se leen linea por linea en la prosa, por eso
% viven aqui y no cuentan contra el limite de paginas del cuerpo.
\appendix
\input{secciones/diccionario_datos.tex}
\input{secciones/diccionario_datos_transformadas.tex}
