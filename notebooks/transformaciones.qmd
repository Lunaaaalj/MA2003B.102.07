...
sima_horario_quant <- sima_horario %>% dplyr::select(-c("estacion","fecha","anio","hora","mes","dia_semana","fin_de_semana","temporada"), -starts_with("outlier_"))
...
La mayoría de las variables no cumplen con la regla propuesta, se convierten en candidatos a transformar, a excepción de `RAINF`,que tiene un sesgo de  144.85, debido a la naturaleza de la precipitación, es una variable inflada de zeros, ni Box-Cox ni Yeo-Johnson funcionaran con esta variable. Para esta variable sera mejor crear una nueva `llovio` que sea la dicotomizacion de la variable original `RAINF`
