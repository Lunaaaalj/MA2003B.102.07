...
(754,603 observaciones horarias, 2020-01-01 a 2025-12-31, 15 estaciones).
...
- El porcentaje de faltantes por parámetro (de 3.7 % en `SR` a 25.2 % en `PM2.5`).
...
La rejilla temporal tiene **52,608 horas** (2,192 días × 24). Si las 15 estaciones
hubieran reportado siempre, habría 789,120 filas; hay 754,603. Pero no es ruido, solo dice cuándo entró en operación cada estación y dónde faltan renglones enteros.
...
Dos estaciones entraron en operación tarde, que son: `NE3` y `NO3`. Lo que explica la diferencia que hay. 43,817 horas (arranca en 2021) y solo 27,045 (arranca a media hoja de
2022, como ya documentaba `data/README.md`).

Pero el resto de las estaciones tampoco llega a las 52,608 horas: pierden entre 3
(`CE`) y 16 (`SUR`) renglones cada una. Con la primera versión de `BD 2025.xlsx`
(cortada al 30 de junio) `SO` perdía 151; la versión completa cerró ese hueco. Eso es una
...
**`-9999` es un código de "sin dato", no una medición.** Son 26 celdas repartidas en
`SR`, `RH`, `NOX`, `TOUT`, `WSR`, `WDR` y `RAINF`. Entró al parquet como número real, así que **cualquier media o desviación
calculada sin quitarlo sale disparatada**. Es el hallazgo más accionable para la #13.

**Fuera del rango de operación** (12,452 observaciones, sospechosas):

| | total | se concentra en |
|---|---|---|
| `PRS` | 8,172 | 2022 (6,305) |
| `WSR` | 1,704 | 2021 (1,184) |
| `SR` | 1,186 | 2022 (717) |
| `TOUT` | 517 | 2020 (450) |
| `RH` | 294 | 2022 (155) |
| `NO` | 284 | 2025 (269) |
...
`NO` y `NOX` son novedad del segundo semestre de 2025: el rango de operación de ese
año baja el tope a 350 y 400 ppb, y la estación `SO` acumula 220 y 159 lecturas por
encima. Son picos de precursores, no imposibles físicamente, así que se marcan y se
conservan, igual que el resto de esta tabla.

**Fuera del rango del fabricante** (imposibles, 1,879 en total): `SR` 1,091 · `TOUT` 405 · `RH` 294 · `WSR` 41 · `NOX` 30 · `NO` 11 · `PRS` 2, más una observación suelta en `PM10`, `PM2.5`, `O3`, `CO` y `WDR`. `NO2` sale limpio.
...
estación `NO`; `SE2` reporta `RH` de 725 % nueve veces entre septiembre y noviembre de
2025. Conviene descartar la observación completa, no el valor suelto.
...
- El **padrón no tiene fecha**, así que entraría como una constante por municipio — la
  misma cifra repetida en las 52,608 horas. No aporta varianza al análisis multivariado.
...
1. **`-9999` está guardado como si fuera una medición** en siete parámetros (26 celdas). Es el
   hallazgo más urgente: contamina cualquier media o desviación. Quitarlo es lo primero
   que debe hacer la #13.
2. **12,452 observaciones caen fuera del rango de operación** de SIMA, concentradas en
   `PRS` (8,172) y `WSR` (1,704). De esas, **1,879 son físicamente imposibles** según el
   rango del fabricante.
3. **Faltan renglones enteros, no solo valores.** Cada estación pierde entre 3 y 16
   horas de la rejilla. Un renglón ausente no cuenta como nulo, así que el % de faltantes reportado
   está subestimado.
