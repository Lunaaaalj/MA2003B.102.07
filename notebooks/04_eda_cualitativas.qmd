...
La construcción vive en `src/agregacion_diaria.py` (#51) y se importa: el
modelado la necesita como predictor, así que dejó de ser código de notebook.

```{python}
from src.agregacion_diaria import marcar_calendario
...
**Verificación contra #14.** `excede_anio_1` debe dar 11.25 % sobre válidos
(2,825 de 25,100). Sobre el total de días-estación daría 10.58 %; la diferencia
...
print(cov.sort_values("n", ascending=False, kind="stable").to_string())
```
...
```{python}
#| label: fig-estacion
#| fig-cap: "Frecuencia por estación y completitud de $O_3$ en paralelo."
c = cov.sort_values("n", kind="stable")
...
# lo mismo sobre NO3.
fig, ax = plt.subplots(1, 2, figsize=(3.4, 3.1), gridspec_kw={"width_ratios": [2, 1]})
...
ax[1].set_xlabel("% horas con O$_3$", fontsize=7); ax[1].set_yticklabels([])
ax[1].set_title("Completitud O$_3$", loc="left", fontsize=8.5)
...
ax.set_title("Ventana de operación (2021-01-01 a 2025-12-31)",
...
Al umbral vigente de 65 ppb las excedencias son el 11.25 % de los
días-estación válidos: un clasificador que prediga siempre la clase
mayoritaria acierta 88.75 % sin capturar nada. La exactitud queda descartada
como métrica en la etapa de clasificación. El umbral de 51 ppb (31.96 %) está
...
chk.append((f"excede_anio_1 = 11.25 % sobre válidos (obtenido {pct:.2f} %)",
            abs(pct - 11.25) < 0.01))
