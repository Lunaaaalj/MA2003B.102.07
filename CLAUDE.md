...
- **`claude-code-review.yml`** — revisión automática, deliberadamente conservadora porque el token está ligado a la suscripción de una persona: corre **una sola vez por PR**, al marcarla como lista (`ready_for_review`, más `opened` para cubrir a quien la abra directamente sin draft; el guard `draft == false` los hace excluyentes). **No** corre en cada push — `synchronize` no está en los `types` — así que una re-revisión se pide a mano con `@claude`, con `--model claude-sonnet-5`, `--max-turns 20`, `timeout-minutes` y `paths-ignore` para PDFs e imágenes.
